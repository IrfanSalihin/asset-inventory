        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <!-- Search Form -->
                    <form method="GET" action="{{ route('laptops.index') }}" class="mb-4 flex flex-wrap -mx-2 space-y-2 md:space-y-0 md:flex-row items-end justify-between">
                        ...
                        <div class="flex w-full md:w-auto px-2 space-x-2">
                            ...
                            <a href="{{ route('laptops.index') }}" class="inline-block bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Clear</a>
                        </div>
                    </form>
                    <!-- Laptop Information -->
                    <div class="flex justify-between items-center mb-4">
                        <h4 class="text-xl font-semibold">Laptop List</h4>
                        <a href="{{ route('laptops.create') }}" class="inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            Add Laptop
                        </a>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $laptop->assigned_staff_name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $laptop->location }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $laptop->status }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('laptops.show', $laptop->id) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                                        <a href="{{ route('laptops.edit', $laptop->id) }}" class="text-blue-600 hover:text-blue-900 ml-4">Edit</a>
                                        <button data-delete-url="{{ route('laptops.destroy', $laptop->id) }}" class="delete-btn text-red-600 hover:text-red-900 ml-4">Delete</button>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
